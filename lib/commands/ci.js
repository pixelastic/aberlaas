...
export default {
  ...
  /**
   * Checks if the PR is a Renovate PR
   * @returns {boolean} True if a Renovate PR
   **/
  isRenovatePR() {
    return this.isPR() && _.startsWith(this.prBranch(), 'renovate/');
  },
  /**
  ...
  async scriptsToRun() {
    ...
    const potentialScripts = this.isRenovatePR()
      ? ['build', 'test']
      : ['build', 'test', 'lint'];
