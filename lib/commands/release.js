...
export default {
  ...
  async run(cliArgs = {}) {
    ...
    // When running tests through this method, Jest does not output anything to
    // the console, so we cannot show which tests fails.
    // So, instead we'll "manually" run the test method inside the release
    // script
    // await this.assertCommand('test', args.test);
