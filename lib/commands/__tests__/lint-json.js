...
describe('lint-json', () => {
  ...
  describe('getInputFiles', () => {
    it('should get json', async () => {
      await firost.write('foo', helper.hostPath('foo.json'));
      await firost.write('foo', helper.hostPath('deep/foo.json'));
  ...
  describe('run', () => {
    it('should return true if all passes', async () => {
      await firost.write('{ "foo": "bar" }', helper.hostPath('foo.json'));
    ...
    it('should throw if a file errors', async () => {
      await firost.write('{ "foo": "bar" }', helper.hostPath('good.json'));
      await firost.write('{ "foo": bar }', helper.hostPath('bad.json'));
    ...
    it('should throw all error message if a file fails', async () => {
      await firost.write('{ "foo": "bar" }', helper.hostPath('good.json'));
      await firost.write('{ "foo": bar }', helper.hostPath('foo.json'));
      await firost.write('{ "foo": bar }', helper.hostPath('deep/bar.json'));
  ...
  describe('fix', () => {
    it('should fix files', async () => {
      ...
      const actual = await firost.read(helper.hostPath('foo.json'));
