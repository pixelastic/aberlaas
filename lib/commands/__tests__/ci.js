...
describe('ci', () => {
  ...
  describe('isRenovatePR', () => {
    it('should return false if not a PR', async () => {
      jest.spyOn(module, 'isPR').mockReturnValue(false);

      const actual = module.isRenovatePR();

      expect(actual).toEqual(false);
    });
    it('should return false if classic PR', async () => {
      jest.spyOn(module, 'isPR').mockReturnValue(true);
      jest.spyOn(module, 'prBranch').mockReturnValue('feature/foo');

      const actual = module.isRenovatePR();

      expect(actual).toEqual(false);
    });
    it('should return true for renovate/ PRs', async () => {
      jest.spyOn(module, 'isPR').mockReturnValue(true);
      jest.spyOn(module, 'prBranch').mockReturnValue('renovate/firost-0.x');
  ...
  describe('scriptsToRun', () => {
    beforeEach(async () => {
      ...
      jest.spyOn(module, 'isRenovatePR').mockReturnValue(false);
    });
    ...
    it('should only run build and test on renovate PRs', async () => {
      jest.spyOn(module, 'isRenovatePR').mockReturnValue(true);

      const actual = await module.scriptsToRun();

      expect(actual).toEqual(['build', 'test']);
    });
