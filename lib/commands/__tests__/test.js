...
describe('test', () => {
  ...
  describe('configFile', () => {
    it('should take the --config CLI option first', async () => {
      ...
      expect(actual).toEqual(helper.hostPath('custom.jest.config.js'));
    });
    it('should allow absolute CLI options', async () => {
      const cliArgs = { config: helper.hostPath('custom.jest.config.js') };
      const actual = await module.configFile(cliArgs);
      expect(actual).toEqual(helper.hostPath('custom.jest.config.js'));
    });
    it('should take the jest.config.js in the host if nothing defined', async () => {
      ...
      expect(actual).toEqual(helper.hostPath('jest.config.js'));
    });
    it('should take the aberlaas default if no jest.config.js in the host', async () => {
      ...
      expect(actual).toEqual(helper.aberlaasPath('configs/jest.js'));
    });
  });
