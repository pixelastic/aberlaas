...
describe('release', () => {
  ...
  describe('run', () => {
    ...
    it('should test by default', async () => {
      const input = {};

      await module.run(input);

      expect(module.assertCommand).toHaveBeenCalledWith('test', true);
    });
    it('should disable test with --no-test', async () => {
      const input = { test: false };

      await module.run(input);

      expect(module.assertCommand).toHaveBeenCalledWith('test', false);
    });
    describe('error', () => {
      beforeEach(() => {
        jest.spyOn(module, '__releaseIt').mockImplementation(() => {
      ...
      it('should warn user that ther is an error', async () => {
  ...
  describe('assertCommand', () => {
    ...
    it('should stop process if command failed', async () => {
      await firost.writeJson(
        { scripts: { build: 'foo' } },
        helper.hostPath('package.json')
      );
