...
describe('init', () => {
  ...
  describe('addConfigFiles', () => {
    ...
    it('should add renovate config file', async () => {
      ...
      const actual = await firost.isFile(
        helper.hostPath('.github/renovate.json')
      );
  ...
  describe('addScripts', () => {
    ...
    it('should add ci script', async () => {
      ...
      const fileCreated = await firost.isFile(helper.hostPath('scripts/ci'));
  ...
  describe('addLicenseFile', () => {
    ...
    it('should create an MIT LICENSE file', async () => {
      ...
      const actual = await firost.isFile(helper.hostPath('LICENSE'));
    ...
    it('should be copyrighted by the author', async () => {
      ...
      const actual = await firost.read(helper.hostPath('LICENSE'));

      expect(actual).toInclude('Copyright (c) Foo <foo@bar.com>');
    });
    it('should not replace existing license', async () => {
      await firost.write('existing license', helper.hostPath('LICENSE'));
  ...
  describe('addDefaultVersion', () => {
    ...
    it('should keep the existing version', async () => {
      ...
      const packageJson = await firost.readJson(packagePath);
      packageJson.version = '1.2.3';
      await firost.writeJson(packageJson, packagePath);

      await module.addDefaultVersion();

      const actual = await firost.readJson(packagePath);
  ...
  describe('pinNodeAndYarn', () => {
    ...
    it('should set a .nvmrc file', async () => {
      ...
      const actual = await firost.read(helper.hostPath('.nvmrc'));
    ...
    it('should run yarn policies', async () => {
      ...
      expect(firost.shell).toHaveBeenCalledWith(
