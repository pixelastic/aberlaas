...
import isFile from 'firost/lib/isFile';
import writeJson from 'firost/lib/writeJson';
import readJson from 'firost/lib/readJson';
...
describe('init', () => {
  ...
  describe('addConfigFiles', () => {
    it('should add babel config file', async () => {
      ...
      const actual = await isFile(helper.hostPath('babel.config.js'));
  ...
  describe('addScaffolding', () => {
    ...
    it('should add ./lib/index.js', async () => {
      ...
      const actual = await isFile(helper.hostPath('./lib/index.js'));
    ...
    it('should add ./lib/__tests__/index.js', async () => {
      ...
      const actual = await isFile(helper.hostPath('./lib/__tests__/index.js'));

      expect(actual).toEqual(true);
    });
    it('should not change files that already exist', async () => {
      await write('foo', helper.hostPath('./lib/index.js'));
      await module.addScaffolding();

      const actual = await read(helper.hostPath('./lib/index.js'));
  ...
  describe('setDefaultReleaseFiles', () => {
    ...
    it('should set .files key to ["lib/"]', async () => {
      await module.setDefaultReleaseFiles();

      const actual = await readJson(helper.hostPath('package.json'));

      expect(actual.files).toContain('lib/');
    });
  });
  ...
  describe('addEsmToDependencies', () => {
    beforeEach(async () => {
      await writeJson({}, helper.hostPath('package.json'));
      jest.spyOn(module, '__run').mockReturnValue();
    });
    it('should run yarn add esm', async () => {
      await module.addEsmToDependencies();

      expect(module.__run).toHaveBeenCalledWith('yarn add esm', {
        stdout: false,
      });
    });
  });
