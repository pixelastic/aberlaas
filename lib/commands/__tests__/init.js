...
describe('init', () => {
  ...
  describe('pinNodeAndYarn', () => {
    ...
    describe('yarn', () => {
      it('should run yarn policies if version is outdated', async () => {
        jest.spyOn(module, 'currentYarnVersion').mockReturnValue('1.19.0');
        jest.spyOn(module, 'expectedYarnVersion').mockReturnValue('1.22.0');
      ...
      it('should not run yarn policies if version is matching', async () => {
        jest.spyOn(module, 'currentYarnVersion').mockReturnValue('1.22.0');
        jest.spyOn(module, 'expectedYarnVersion').mockReturnValue('1.22.0');
    ...
    describe('engines.node version', () => {
      it('should set the minimum version if not set', async () => {
        await writeJson({}, helper.hostPath('package.json'));
        ...
        const expected = `>=${module.nodeVersion}`;
        expect(actual).toHaveProperty('engines.node', expected);
      });
      it('should not change the minimum version if set', async () => {
        await writeJson(
          { engines: { node: '>=0.0.1' } },
          helper.hostPath('package.json')
        );
