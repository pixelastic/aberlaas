...
describe('lint-js', () => {
  ...
  describe('getCliArguments', () => {
    it('should pass input files directly', async () => {
      ...
      const actual = await module.getCliArguments(input);
    ...
    it('should allow specifying the config file', async () => {
      ...
      const actual = (await module.getCliArguments(input)).join(' ');
  ...
  describe('run', () => {
    ...
    it('should spawn eslint binary with cli options', async () => {
      ...
      jest.spyOn(module, 'getCliArguments').mockReturnValue('args');
      await module.run();
