...
describe('ci > autoRelease', () => {
  ...
  describe('configureGit', () => {
    beforeEach(async () => {
      jest.spyOn(module, 'gitRun').mockReturnValue();
    ...
    it('should set the user name', async () => {
      ...
      expect(module.gitRun).toHaveBeenCalledWith(
  ...
  describe('configureNpm', () => {
    ...
    it('should create a .npmrc file', async () => {
      await module.configureNpm();
      const actual = await read(helper.hostPath('.npmrc'));
      expect(actual).toInclude('NPM_TOKEN');
    });
  });
});
