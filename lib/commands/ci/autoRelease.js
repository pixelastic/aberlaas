...
module.exports = {
  ...
  async configureGit() {
    ...
    await this.gitRun(`git config user.email ${email}`);
    await this.gitRun(`git config user.name ${name}`);
  },
  /**
  ...
  async configureNpm() {
    ...
    const path = helper.hostPath('.npmrc');
    await this.__write(content, path);
  },
  /**
