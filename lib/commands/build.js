...
import mkdirp from 'firost/lib/mkdirp';
...
export default {
  ...
  async babelCliArguments(cliArgs) {
    // Input files
    const inputFiles = _.isEmpty(cliArgs._) ? ['./lib'] : cliArgs._;
  ...
  async run(cliArgs = {}) {
    ...
    const outputDir = this.outputDir(cliArgs);
    await this.__mkdirp(outputDir);
    await this.__run(`${binary} ${options.join(' ')}`);
  },
  __mkdirp: mkdirp,
  __run: run,
};
