...
import lintCircleCi from './lint-circleci';
...
export default {
  ...
  async run(cliArgs) {
    ...
    await pMap(typesToLint, async type => {
      ...
      try {
        ...
        firost.consoleError(error.message);
        hasErrors = true;
      }
    });

    if (hasErrors) {
      throw firost.error('ERROR_LINT', 'Error while linting files');
    }
  ...
  async fixWithPrettier(inputFiles) {
    ...
    try {
      await firost.run(`${binary} ${options.join(' ')}`);
    } catch {
      throw firost.error(
