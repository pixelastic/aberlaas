...
export default {
  ...
  async fixWithPrettier(inputFiles) {
    ...
    const errors = [];
    // Executing prettier --fix on each file
    await pMap(inputFiles, async file => {
      const options = ['--write', '--loglevel=silent', file];
      try {
        await this.__execa(binary, options);
      } catch {
        const relativePath = path.relative(helper.hostRoot(), file);
        errors.push(`./${relativePath}`);
      }
    });

    // If any errored, we throw with the list of errors
    if (!_.isEmpty(errors)) {
      const displayErrors = errors.join('\n');
      throw helper.error(
        'LINT_ERROR_FIX_PRETTIER',
        `The following files have errors that can't be fixed automatically:\n${displayErrors}`
      );
    }
  },
  __execa: execa,
