...
describe('init', () => {
  beforeEach(async () => {
    vi.spyOn(helper, 'hostRoot').mockReturnValue('./tmp/init');
  ...
  describe('addConfigFiles', () => {
    it('should add eslint config file', async () => {
      ...
      const actual = await isFile(helper.hostPath('.eslintrc.cjs'));
    ...
    it('should add vitest config file', async () => {
      await current.addConfigFiles();

      const actual = await isFile(helper.hostPath('vite.config.js'));
    ...
    it('should add husky config file', async () => {
      ...
      const actual = await isFile(helper.hostPath('.huskyrc.cjs'));
    ...
    it('should add stylelint config file', async () => {
      ...
      const actual = await isFile(helper.hostPath('.stylelintrc.cjs'));
    ...
    it('should add prettier config file', async () => {
      ...
      const actual = await isFile(helper.hostPath('.prettierrc.cjs'));
  ...
  describe('addDefaultName', () => {
    ...
    it('should add the dirname if no name defined', async () => {
      ...
      expect(actual).toHaveProperty('name', 'init');
    });
  });
