...
export default {
  ...
  async pinNodeAndYarn() {
    ...
    await firost.write(this.nodeVersion, nvmrcPath);

    // Download exact yarn version
    await firost.shell(`yarn policies set-version ${this.yarnVersion}`);

    // Update package.json
    const packagePath = helper.hostPath('package.json');
    const packageContent = await firost.readJson(packagePath);
    const newContent = {
      ...packageContent,
      engines: {
    ...
    await firost.writeJson(newContent, packagePath);
  },
  /**
  ...
  async run() {
    ...
    progress.tick('Update LICENSE');
    await this.addLicenseFile();

    progress.tick('Set default release files');
    await this.setDefaultReleaseFiles();

    progress.tick('Pin node and yarn versions');
    await this.pinNodeAndYarn();

    progress.tick('aberlaas project initialized');
  },
};
