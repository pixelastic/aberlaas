...
module.exports = {
  nodeVersion: '12.12.0', // Also see templates/_circleci/config.yml
  yarnVersion: '1.22.4',
  ...
  async addConfigFiles() {
    ...
    // Babel
    // Even if not used directly, it is still used by Jest
  ...
  async pinNodeAndYarn() {
    ...
    const currentYarnVersion = await this.currentYarnVersion();
    const expectedYarnVersion = this.expectedYarnVersion();
    if (currentYarnVersion !== expectedYarnVersion) {
      await this.__run(`yarn policies set-version ${expectedYarnVersion}`, {
        stdout: false,
      });
    }

    // Set default engine in package.json
    const packagePath = helper.hostPath('package.json');
    const currentPackage = await readJson(packagePath);
    if (_.has(currentPackage, 'engines.node')) {
      return;
    }
    const nodeEngine = `>=${this.nodeVersion}`;
    const newPackage = _.merge(currentPackage, {
  ...
  /**
   * Return the current Yarn version
   * @returns {string} Yarn version
   **/
  async currentYarnVersion() {
    return await this.__run('yarn --version', { stdout: false });
  },
  /**
   * Returns the expected yarn version
   * @returns {string} Yarn version
   **/
  expectedYarnVersion() {
