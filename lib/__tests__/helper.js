...
describe('module', () => {
  ...
  describe('aberlaasRoot', () => {
    it('should return the aberlaas root', async () => {
      ...
      const actual = await firost.readJson(`${path}/package.json`);
  ...
  describe('yarnRun', () => {
    beforeEach(async () => {
      ...
      await firost.emptyDir(module.hostRoot());
    });
    ...
    it('should throw an error if the script fails', async () => {
      await firost.writeJson(
        { license: 'MIT', scripts: { build: 'exit 42' } },
        module.hostPath('package.json')
      );
      ...
      expect(actual).toHaveProperty(
        ...
        expect.stringContaining(
          'Command failed with exit code 42: yarn run build'
        )
      );
