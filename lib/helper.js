...
export default {
   ...
   **/
  async which(bin) {
    const pathsToTest = [this.aberlaasRoot(), this.hostRoot()];

    // Note: We run the checks in parallel and then compare the outputs instead
    // of checking them sequentially to improve performance.
    // Sure, we could sometimes skip checking the host if we already found the
    // binary in aberlaas, if we don't, checking both in sequence doubles the
    // time to execute this function.
    const results = await pMap(pathsToTest, async (pathToTest) => {
      const output = await run(`yarn bin ${bin}`, {
        cwd: pathToTest,
        stdout: false,
        stderr: false,
      });
    ...
    return _.compact(results)[0] || null;
  },
  /**
